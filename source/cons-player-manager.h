...
public:
	...
	float PlayGame(ConsPlayer const * whitePlayer, ConsPlayer const * blackPlayer);
	// returns true if the game is complete (there are no valid moves), false otherwise
	bool PlayMove(GameState const & gameState, GameState * nextGameState);
	float MinimaxScoreNode(GameStateNode const & node, GameStateNode const * nodes);
	void EvaluateGameState(GameState const & state, GameState const & previousState, Tile playerColor, int_fast32_t depth, float * tv, float * cv);
